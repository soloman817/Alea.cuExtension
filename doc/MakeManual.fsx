#r @"..\misc\fsformatting\FSharp.PowerPack.Metadata.dll"
...
type Options private () =
    ...
    let mutable sampleListFile : string = @"Alea.cuBase.Sample.All.list"
...
let projectName = "Alea.cuBase Manual"
...
let productVersion() =
    if Options.Instance.NoReference then "x.x.x"
        ...
        let assemblyFolder = Options.Instance.TryAssemblyFolder |> function
            | Some folder -> folder
            | None -> Options.Instance.SolutionFolder @@ "src" @@ "Alea.CUDA" @@ "bin" @@ "Release"

        let assemblyPath = assemblyFolder @@ "Alea.CUDA.dll"
    ...
    let rec create (parent:Document option) (srcDir:string) =
        ...
        for file in (Directory.GetFiles(srcDir)) do
            ...
            | "SAMPLES", ".reference" ->
                ...
                |> Array.filter (fun sample -> sample <> "SimpleD3D9")
                |> Array.filter (fun sample -> sample <> "HeatPDE")
                |> Array.iteri (fun i sample ->
                    ...
                    //printfn "Loading %s..." assemblyFile
                    //FSharpAssembly.FromFile(assemblyFile) |> ignore                
