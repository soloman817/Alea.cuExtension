...
let action (f:ActionHint -> unit) = PCalc(fun s -> s.AddAction(f); (), s)
let force () = PCalc(fun s -> s.RunActions(); (), s)
let stream (stream:Stream) = PCalc(fun s -> s.ActionHint <- { s.ActionHint with Stream = stream }; (), s)
let streamWithHint (stream:Stream) (totalStreams:int) = PCalc(fun s -> s.ActionHint <- { s.ActionHint with Stream = stream; TotalStreams = Some totalStreams }; (), s)
