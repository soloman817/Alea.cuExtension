...
//let serialMerge (VT:int) (rangeCheck:int) (compOp:CompOpType)=
//    <@ fun (keys_shared:RWPtr<'T>) (aBegin:int) (aEnd:int) (bBegin:int) (bEnd:int) (results:RWPtr<'T>) (indices:RWPtr<int>) ->
//        let aKey = keys_shared.[aBegin]
//        let bKey = keys_shared.[bBegin]
//
//        for i = 0 to VT - t do
//            let mutable p = 0
//            if rangeCheck = 1 then
//                p <- if (bBegin >= bEnd) || ((aBegin < aEnd) && !comp(bKey, aKey)) then 1 else 0
//            else
...
//    member fmi.Host =
//        fun (frame:int3) coop block nv count mp0 mpl ->
