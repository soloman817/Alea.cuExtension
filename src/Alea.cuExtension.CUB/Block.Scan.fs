...
//let inline InternalBlockScan<'T>() =
//    fun block_threads algorithm ->
//        match (block_threads, algorithm) ||> SAFE_ALGORITHM with
//        | BLOCK_SCAN_WARP_SCANS -> BlockScanWarpScans.BlockScanWarpScans<'T>.Create
//        | _ -> BlockScanRaking.BlockScanRaking<'T>.Create
...
type internal Constants =
    ...
    static member Init(tp:TemplateParameters) =
        {
            SAFE_ALGORITHM = 
                if ((tp.ALGORITHM = BLOCK_SCAN_WARP_SCANS) && ((tp.BLOCK_THREADS % CUB_PTX_WARP_THREADS) <> 0)) then
                    BLOCK_SCAN_RAKING
                else
                    tp.ALGORITHM
        }
...
[<Record>]
type InternalBlockScan<'T> =
    {   
        BlockScanWarpScans      : BlockScanWarpScans<'T>
        BlockScanRaking         : BlockScanRaking<'T>
    }


    static member Init(block_threads:int, c:Constants) =
        {
            BlockScanWarpScans = block_threads |> BlockScanWarpScans<'T>.Create
            BlockScanRaking = (block_threads, (c.SAFE_ALGORITHM = BLOCK_SCAN_RAKING_MEMOIZE)) |> BlockScanRaking<'T>.Create
        }

    member this.Get(c:Constants) = //, temp_storage, linear_tid) =
...
type BlockScan<'T> =
    {
        ...
        Constants           : Constants
        InternalBlockScan   : InternalBlockScan<'T>
    ...
    member this.ExclusiveSum(input:'T, output:Ref<'T>) =
        ...
        let InternalBlockScan = this.InternalBlockScan.Get(this.Constants) |> function
            | Some, None ->  
        this.InternalBlockScan(temp_storage, linear_tid).ExclusiveSum(input, output, block_aggregate)
    ...
    member this.ExclusiveSum(input:'T, output:Ref<'T>, block_aggregate:Ref<'T>, block_prefix_callback_op:Ref<'BlockPrefixCallbackOp>) =
        this.InternalBlockScan(temp_storage, linear_tid).ExclusiveSum(input, output, block_aggregate, block_prefix_callback_op)
    ...
    member this.ExclusiveScan(input:'T, output:Ref<'T>, identity:'T, scan_op:IScanOp<'T>) =
        InternalBlockScan(temp_storage, linear_tid).ExclusiveScan(input, output, identity, scan_op, block_aggregate)

    member this.ExclusiveScan(input:'T, output:Ref<'T>, identity:Ref<'T>, scan_op:IScanOp<'T>, block_aggregate:Ref<'T>) =
    ...
    member this.ExclusiveScan(input:'T, output:Ref<'T>, identity:'T, scan_op:IScanOp<'T>, block_aggregate:Ref<'T>, block_prefix_callback_op:Ref<'BlockPrefixCallbackOp>) =
    ...
    member this.ExclusiveScan(input:'T, output:Ref<'T>, scan_op:IScanOp<'T>) =
        let thread_partial = ThreadReduce(input, scan_op)
    ...
    member this.ExclusiveScan(input:'T, output:Ref<'T>, scan_op:IScanOp<'T>, block_aggregate:Ref<'T>) =
        let thread_partial = ThreadReduce(input, scan_op)
    ...
    member this.ExclusiveScan(input:'T, output:Ref<'T>, scan_op:IScanOp<'T>, block_aggregate:Ref<'T>, block_prefix_callback_op:Ref<'BlockPrefixCallbackOp>) =
        let block_aggregate = __null() |> __ptr_to_ref
    ...
    //<items_per_thread>
    member this.ExclusiveScan(input:deviceptr<'T>, output:deviceptr<'T>, identity:Ref<'T>, scan_op:IScanOp<'T>) =
        this.InternalBlockScan(temp_storage, linear_tid).ExclusiveScan(input, output, scan_op, block_aggregate)

    member this.ExclusiveScan(input:deviceptr<'T>, output:deviceptr<'T>, identity:Ref<'T>, scan_op:IScanOp<'T>, block_aggregate:Ref<'T>) =
        InternalBlockScan(temp_storage, linear_tid).ExclusiveScan(input, output, scan_op, block_aggregate, block_prefix_callback_op)

    member this.ExclusiveScan(input:deviceptr<'T>, output:deviceptr<'T>, identity:Ref<'T>, scan_op:IScanOp<'T>, block_aggregate:Ref<'T>, block_prefix_callback_op:Ref<'BlockPrefixCallbackOp>) =
        let thread_partial = ThreadReduce(input, scan_op)
    ...
    //<items_per_thread>
    member this.ExclusiveScan(input:deviceptr<'T>, output:deviceptr<'T>, scan_op:IScanOp<'T>) =
    ...
    member this.ExclusiveScan(input:deviceptr<'T>, output:deviceptr<'T>, scan_op:IScanOp<'T>, block_aggregate:Ref<'T>) =
        // Reduce consecutive thread items in registers
        let thread_partial = ThreadReduce(input, scan_op)
    ...
    member this.ExclusiveScan(input:deviceptr<'T>, output:deviceptr<'T>, scan_op:IScanOp<'T>, block_aggregate:Ref<'T>, block_prefix_callback_op:Ref<'BlockPrefixCallbackOp>) =
        let block_aggregate = __null() |> __ptr_to_ref
    ...
    member this.InclusiveScan(input:'T, output:Ref<'T>, scan_op:IScanOp<'T>) =
        let block_aggregate = __null() |> __ptr_to_ref
        this.InclusiveScan(input, output, scan_op, block_aggregate)
    
    member this.InclusiveScan(input:'T, output:Ref<'T>, scan_op:IScanOp<'T>, block_aggregate:Ref<'T>) =
        InternalBlockScan(temp_storage, linear_tid).InclusiveScan(input, output, scan_op, block_aggregate)


    member this.InclusiveScan(input:'T, output:Ref<'T>, scan_op:IScanOp<'T>, block_aggregate:Ref<'T>, block_prefix_callback_op:Ref<'BlockPrefixCallbackOp>) =
    ...
    //<items_per_thread>
    member this.InclusiveScan(input:deviceptr<'T>, output:deviceptr<'T>, scan_op:IScanOp<'T>) =
        fun (items_per_thread:int) ->
           if (items_per_thread = 1) then
    ...
    member this.InclusiveScan(input:deviceptr<'T>, output:deviceptr<'T>, scan_op:IScanOp<'T>, block_aggregate:Ref<'T>) =
        fun (items_per_thread:int) ->
            if (items_per_thread = 1) then
    ...
    member this.InclusiveScan(input:deviceptr<'T>, output:deviceptr<'T>, scan_op:IScanOp<'T>, block_aggregate:Ref<'T>, block_prefix_callback_op:Ref<'BlockPrefixCallbackOp>) =
        fun (items_per_thread:int) ->
            if (items_per_thread = 1) then
    ...
    static member Create(tp:TemplateParameters<'T>) =
        {
            TemplateParameters = tp
            Constants =
            ThreadFields = ThreadFields<'T>.Init(__null, threadIdx.x)
...
////        // exclusive prefix scan operations
////        let exclusiveScan =
////            fun (input:'T) (output:Ref<'T>) (identity:Ref<'T>) (scan_op:IScanOp<'T>) (block_aggregate:Ref<'T> option) (block_prefix_callback_op:Ref<'BlockPrefixCallbackOp> option) -> ()
////
////
////        module Identityless =
////            // exclusive prefix scan operations (identityless, single datum per thread)
////            let exclusiveScan =
////                fun (input:'T) (output:Ref<'T>) (scan_op:IScanOp<'T>) (block_aggregate:Ref<'T> option) (block_prefix_callback_op:Ref<'BlockPrefixCallbackOp> option) -> ()   
////
////
////    module STMD =
////        // exclusive prefix sum operations (multiple data per thread)
...
////            fun (input:deviceptr<'T>) (output:deviceptr<'T>) (identity:Ref<'T>) (scan_op:IScanOp<'T>) (block_aggregate:Ref<'T> option) (block_prefix_callback_op:Ref<'BlockPrefixCallbackOp> option) -> ()
////        
////
////        module Identityless =
////            // exclusive prefix scan operations (identityless, multiple data per thread)
////            let exclusiveScan_noId items_per_thread =
////                fun (input:deviceptr<'T>) (output:deviceptr<'T>) (scan_op:IScanOp<'T>) (block_aggregate:Ref<'T> option) (block_prefix_callback_op:Ref<'BlockPrefixCallbackOp> option) -> ()
////
////
