...
module Template =
    module Host =
        ...
        type API =
            ...
            static member Init(block_threads, items_per_thread, algorithm, warp_time_slicing) =
                ...
                { Params = p; SharedMemoryLength = sml }
    ...
    module Device =
        ...
        module ThreadFields =
            ...
            type API<'T> =
                ...
                [<ReflectedDefinition>] 
                static member Init(temp_storage:TempStorage.API<'T>, linear_tid)    = { temp_storage = temp_storage; f.linear_tid = f.linear_tid }
                [<ReflectedDefinition>] 
                static member Init(length)                              = { temp_storage = TempStorage.API<'T>.Uninitialized(length); f.linear_tid = 0 }
        ...
        type API<'T> =
            ...
            [<ReflectedDefinition>] 
            static member Init(sharedMemoryLength) = 
                let temp_storage = BlockExchange.TempStorage<'T>.Uninitialized(sharedMemoryLength)
                { ThreadFields = ThreadFields.API<'T>.Init(temp_storage, 0) }
            [<ReflectedDefinition>] 
            static member Init(temp_storage:BlockExchange.TempStorage<'T>) = 
                { ThreadFields = ThreadFields.API<'T>.Init(temp_storage, 0) }
    ...
    type _FunctionApi<'T> =
        {
            LoadDirectBlocked : int
            LoadDirectBlockedVectorized : int
            LoadDirectStriped : int
            LoadDirectWarpStriped : int
        }


module private LoadDirectBlocked =
    open Template
    ...
    let Guarded (h:_HostApi) =
        <@ fun  (d:_DeviceApi<'T>)
                (block_itr:deviceptr<'T>) (items:deviceptr<'T>) (valid_items:int) ->
            let p = h.Params
            let f = d.ThreadFields

            let bounds = valid_items - (f.linear_tid * p.ITEMS_PER_THREAD)
            for ITEM = 0 to (p.ITEMS_PER_THREAD - 1) do items.[ITEM] <- block_itr.[(linear_tid * p.ITEMS_PER_THREAD) + ITEM]
        @>
        
    let GuardedWithOOB (h:_HostApi) =
        <@ fun  (d:_DeviceApi<'T>)
    ...
    let api (h:_HostApi) = (Default h, Guarded h, GuardedWithOOB h)


module private LoadDirectBlockedVectorized =
    open Template
    ...
    let Default (h:_HostApi) =
        <@ fun  (d:_DeviceApi<'T>)
            ...
            let ptr = (block_ptr + (f.linear_tid * VEC_SIZE * VECTORS_PER_THREAD)) |> __ptr_reinterpret
...
module private LoadDirectStriped =
    open Template
    ...
    let Guarded (h:_HostApi) =
        <@ fun  (d:_DeviceApi<'T>)
            ...
            let bounds = valid_items - f.linear_tid
    ...
    let GuardedWithOOB (h:_HostApi) =
        <@ fun (d:_DeviceApi<'T>) (block_itr:deviceptr<'T>) (items:deviceptr<'T>) (valid_items:int) (oob_default:'T) ->
            let p = h.Params
            let f = d.ThreadFields
...
module private LoadDirectWarpStriped =
    open Template
    ...
    let Default (h:_HostApi) =
        <@ fun  (d:_DeviceApi<'T>)
            ...
            let tid = f.linear_tid &&& (CUB_PTX_WARP_THREADS - 1)
            let wid = f.linear_tid >>> CUB_PTX_LOG_WARP_THREADS
...
module private LoadInternal =
    ...
    module BlockLoadDirect =
        let Default (h:_HostApi) =
            <@ fun  (d:_DeviceApi<'T>)
                    (block_ptr:deviceptr<'T>) (items:deviceptr<'T>) ->
                LoadDirectBlocked.Default template f.linear_tid block_ptr items
            @>

        let Guarded (h:_HostApi) =
            let LoadDirectBlocked = h |> LoadDirectBlocked.Guarded
            <@ fun  (d:_DeviceApi<'T>)
                    (block_ptr:deviceptr<'T>) (items:deviceptr<'T>) (valid_items:int) ->
                LoadDirectBlockedGuarded template f.linear_tid block_ptr items valid_items
            @>

        let GuardedWithOOB (h:_HostApi) = 
            let LoadDirectBlocked = h |> LoadDirectBlocked.GuardedWithOOB
            <@ fun  (d:_DeviceApi<'T>)
                    (block_ptr:deviceptr<'T>) (items:deviceptr<'T>) (valid_items:int) (oob_default:'T) ->
                (%LoadDirectBlocked) d block_ptr items valid_items oob_default
            @>
    ...
    module BlockLoadVectorized =
        let Default (h:_HostApi) =
            let LoadDirectBlockedVectorized = h |> LoadDirectBlockedVectorized.Default
            <@ fun  (d:_DeviceApi<'T>)
                    (block_ptr:deviceptr<'T>) (items:deviceptr<'T>) ->
                (%LoadDirectBlockedVectorized) d block_ptr items
            @>
        ...
        let Guarded (h:_HostApi) =
            ...
            <@ fun  (d:_DeviceApi<'T>)
                    (block_ptr:deviceptr<'T>) (items:deviceptr<'T>) (valid_items:int) ->
                (%LoadDirectBlocked) d block_ptr items valid_items
            @>
    ...
    module BlockLoadTranspose =
        let private StripedToBlocked (h:_HostApi) =
            let p = h.Params
            let stb =
                BlockExchange.HostApi.Init(p.BLOCK_THREADS, p.ITEMS_PER_THREAD, p.WARP_TIME_SLICING)
                |> StripedToBlocked.api
            <@ fun  (items:deviceptr<'T>) ->
                (%stb)
                <|  BlockExchange.DeviceApi<'T>.Init(h.SharedMemoryLength)
                <| items
            @>

        let private Default (h:_HostApi) =
            let StripedToBlocked    = h |> StripedToBlocked
            let LoadDirectStriped   = h |> LoadDirectStriped.Default
            <@ fun  (d:_DeviceApi<'T>)
                    (block_ptr:deviceptr<'T>) (items:deviceptr<'T>) ->
                (%LoadDirectStriped) d block_ptr items
                (%StripedToBlocked) items
            @>

        let private Guarded (h:_HostApi) =
            let StripedToBlocked    = h |> StripedToBlocked
            let LoadDirectStriped   = h |> LoadDirectStriped.Guarded
            <@ fun  (d:_DeviceApi<'T>)
                    (block_ptr:deviceptr<'T>) (items:deviceptr<'T>) (valid_items:int) ->
                (%LoadDirectStriped) d block_ptr items valid_items
                (%StripedToBlocked) items
            @>

        let private GuardedWithOOB (h:_HostApi) =
            let StripedToBlocked    = h |> StripedToBlocked
            let LoadDirectStriped   = h |> LoadDirectStriped.GuardedWithOOB
            <@ fun  (d:_DeviceApi<'T>)
                    (block_ptr:deviceptr<'T>) (items:deviceptr<'T>) (valid_items:int) (oob_default:'T) ->
                (%LoadDirectStriped) d block_ptr items valid_items oob_default
                (%StripedToBlocked) items
            @>
    ...
    module BlockLoadWarpTranspose =
        let WarpStripedToBlocked (h:_HostApi) =
            let p = h.Params
            let wstb =
                    BlockExchange.HostApi.Init(p.BLOCK_THREADS, p.ITEMS_PER_THREAD, p.WARP_TIME_SLICING)
                    |> WarpStripedToBlocked.api
            <@ fun  (items:deviceptr<'T>) ->
                (%wstb)
                <|  BlockExchange.DeviceApi<'T>.Init(h.SharedMemoryLength)
                <|  items
        ...
        let Default (h:_HostApi) =
            let WarpStripedToBlocked    = h |> WarpStripedToBlocked
            let LoadDirectWarpStriped   = h |> LoadDirectWarpStriped.Default
            <@ fun  (d:_DeviceApi<'T>)
                    (block_ptr:deviceptr<'T>) (items:deviceptr<'T>) ->
                (%LoadDirectWarpStriped) d block_ptr items
                (%WarpStripedToBlocked) items
            @>

        let Guarded (h:_HostApi) =
            let WarpStripedToBlocked    = h |> WarpStripedToBlocked
            let LoadDirectWarpStriped   = h |> LoadDirectWarpStriped.Guarded
            <@ fun  (d:_DeviceApi<'T>)
                    (block_ptr:deviceptr<'T>) (items:deviceptr<'T>) (valid_items:int) ->
                (%LoadDirectWarpStriped) d block_ptr items valid_items
                (%WarpStripedToBlocked) items
            @>

        let GuardedWithOOB (h:_HostApi) =
            let WarpStripedToBlocked    = h |> WarpStripedToBlocked
            let LoadDirectWarpStriped   = h |> LoadDirectWarpStriped.GuardedWithOOB
            <@ fun  (d:_DeviceApi<'T>)
                    (block_ptr:deviceptr<'T>) (items:deviceptr<'T>) (valid_items:int) (oob_default:'T) ->
                (%LoadDirectWarpStriped) d block_ptr items valid_items oob_default
                (%WarpStripedToBlocked) items
            @>

        let api (h:_HostApi) = 
            let WARP_THREADS = CUB_PTX_WARP_THREADS
            if (h.Params.BLOCK_THREADS % WARP_THREADS) <> 0 then failwith "BLOCK_THREADS must be a multiple of WARP_THREADS"
            (Default h, Guarded h, GuardedWithOOB h)
    ...
    let api (h:_HostApi) =
        ...
        let _Default, _Guarded, _GuardedWithOOB = 
            ...
            | _ -> failwith ""
        (_Default, _Guarded, _GuardedWithOOB)
...
module BlockLoad =
    ...
    type API<'T> =
        ...
        member this.Init() =
            ...
            f.temp_storage  <- PrivateStorage()
            f.linear_tid    <- threadIdx.x
            
        [<ReflectedDefinition>]
        member this.Init(temp_storage:TempStorage<'T>) =
            let mutable f = this.device.ThreadFields
            f.temp_storage  <- temp_storage
            f.linear_tid    <- threadIdx.x
        ...
        [<ReflectedDefinition>]
        member this.Init(temp_storage:TempStorage<'T>, linear_tid:int) =
            let mutable f = this.device.ThreadFields
            f.temp_storage  <- temp_storage
            f.linear_tid    <- linear_tid


    let template<'T> (block_threads:int) (items_per_thread:int) (algorithm:BlockLoadAlgorithm) (warp_time_slicing:bool) = cuda {
...
//                    (%Default) template f.linear_tid block_ptr items
//                
//        
//            let! _Guarded =
//                let Guarded = LoadInternal.Guarded
//                <@ fun (d:_DeviceApi<'T>)(block_ptr:deviceptr<'T>) (items:deviceptr<'T>) (valid_items:int) ->
//                    let template = _Template<'T>.Init(h, algorithm)
//                    (%Guarded) template f.linear_tid block_ptr items valid_items
//                @> |> Compiler.DefineFunction
//        
//            let! _GuardedWithOOB =
//                let GuardedWithOOB = LoadInternal.GuardedWithOOB
//                <@ fun (d:_DeviceApi<'T>)(block_ptr:deviceptr<'T>) (items:deviceptr<'T>) (valid_items:int) (oob_default:'T) ->
//                    let template = _Template<'T>.Init(h, algorithm)
//                    (%GuardedWithOOB) template f.linear_tid block_ptr items valid_items oob_default
//                @> |> Compiler.DefineFunction
//        
//            
//                { 
...
//                }
    ...
    let inline template<'T> (block_threads:int) (items_per_thread:int) (algorithm:BlockLoadAlgorithm) (warp_time_slicing:bool) = cuda {
        ...
        let! _Default =
            ...
            <@ fun (d:_DeviceApi<'T>)(block_ptr:deviceptr<'T>) (items:deviceptr<'T>) ->
                Default template f.linear_tid block_ptr items
            @> |> Compiler.DefineFunction
...
//        fun temp_storage f.linear_tid warp_lane warp_id warp_offset ->
//            {
...
//    match temp_storage, f.linear_tid with
//    | Some temp_storage, Some f.linear_tid -> temp_storage,       linear_tid
//    | None,              Some f.linear_tid -> privateStorage(),   linear_tid
...
//            mutable f.linear_tid : int option
...
//                temp_storage = None;
//                f.linear_tid = f.linear_tid |> Some;
