...
let testMergeKeys() =
    ...
    let values1 n na nb = 
        let (hA:int[]) = rngGenericArrayBounded na n |> Array.sort
        let (hB:int[]) = rngGenericArrayBounded nb n |> Array.sort        
        hA,hB
    ...
    let values3 n na nb = 
        let (hA:float[]) = rngGenericArrayBounded na n |> Array.sort
        let (hB:float[]) = rngGenericArrayBounded nb n |> Array.sort        
        hA,hB
...
//                                                                  values3 ns na nb ||> test <| compOp |> PCalc.run)
    ...
    let n = 2097152
    let na,nb = aib n
    let test = (values1 n na nb) ||> test false eps <| compOp
